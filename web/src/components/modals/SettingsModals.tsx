...
const NUMERIC_KNOBS: Array<[string, string, number, number, number, string]> = [
  ...
  ['react_tool_timeout_seconds', '工具单步超时(秒)', 0, 3600, 30,
    'ReAct 路径单次工具调用的等待上限。工具"挂住不返回"（等网络/等子进程/'
    + '等系统锁）时中止这一步并把原因喂回模型，任务继续而不是停在那里；'
    + '0 = 不限制（旧行为）。模型可在参数里用 timeout 申请更长，上限见下方'],
  ['max_task_seconds', '整轮任务总时长(秒)', 0, 28800, 300,
    '单次任务的总时长预算。到点不再开启新步骤，已完成的工作全部保留并给出'
    + '部分交付清单；0 = 不限'],
  ['goal_timeout_seconds', '单步执行上限(秒)', 0, 7200, 60,
    '并行批次里单个步骤的上限。步骤"卡住不返回"（等网络/等子进程/等人审批）'
    + '时据此中止该步、保住同批其它步骤的成果；0 = 不限制'],
  ['react_tool_budget', '每步下发工具数', 0, 40, 2,
    'ReAct 每一步都要重发工具 schema（31 个约 6k~8k token/步）。'
    + '0 = 全量下发；未下发的工具仍在目录里，模型点名后会补发'],
  ['react_repeat_action_threshold', '重复动作提示阈值', 1, 10, 1,
    '连续第 N 次执行完全相同的动作时先提示、再重复则拦截。'
    + '这是"原地打转"的主要形态：不报错、只是反复读同一个文件'],
  ['react_no_progress_limit', '无进展收尾阈值', 0, 50, 1,
    '重复动作被拦截到该次数即提前收尾并给出部分交付清单；0 = 只拦不提前收尾'],
