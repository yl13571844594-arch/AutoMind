...
- **GitHub Actions**：`.github/workflows/desktop-build.yml`
  （手动 Run 或推 `v*.*.*` tag 触发）产出：
  - Windows → `AutoMind-Setup-<ver>.exe`
  - macOS → `AutoMind-<ver>.dmg`（通用二进制，Apple Silicon + Intel）
  - Linux → `automind_<ver>_amd64.deb`（Debian/Ubuntu）
...
# macOS（在 universal2 Python 上出通用二进制）
pyinstaller automind.spec --noconfirm      # → dist/AutoMind.app
...
`~/.local/share/automind`（`automind/core/paths.py` 统一解析）。macOS 未
公证时用户首次需右键 `.app` →「打开」；配 `AUTOMIND_MAC_CODESIGN_IDENTITY`
（Developer ID）即深度签名。
